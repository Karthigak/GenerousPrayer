...
- (void)viewDidLoad {
  ...
  [self.pageViewController didMoveToParentViewController:self];
   self.view.backgroundColor = [CommonMethodClass pxColorWithHexValue:@"#2A50AB"];
}
...
- (void)reloadData {
  ...
  frame.origin.y = [self headerHeight]+60;
  frame.size.height -= [self headerHeight]+60;
...
- (void)reloadTabs {
  ...
  if ([[self dataSource] respondsToSelector:@selector(viewForTabAtIndex:)]) {
    ...
    if ([[self dataSource] respondsToSelector:@selector(titleFont)]) {
      ...
      font = [UIFont fontWithName:@"Myriad Pro-Regular" size:17.0f];
    }
