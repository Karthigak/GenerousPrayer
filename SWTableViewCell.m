...
- (void)initializer
{
   ...
   _longPressGestureRecognizer = [[SWLongPressGestureRecognizer alloc] initWithTarget:self action:@selector(scrollViewPressed:)];
    ...
    [self.cellScrollView addGestureRecognizer:_longPressGestureRecognizer];
    ...
    for (NSUInteger i = 0; i < 2; ++i)
    {
        ...
        [self addConstraints:@[
                               ...
                               [NSLayoutConstraint constraintWithItem:buttonView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:clipView attribute:NSLayoutAttributeTop multiplier:.5 constant:0.0],
                               ...
                               [NSLayoutConstraint constraintWithItem:buttonView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationLessThanOrEqual toItem:self.contentView attribute:NSLayoutAttributeWidth multiplier:width constant:-kUtilityButtonWidthDefault],
                               ]];
    }
}
...
-(void)customArray:(NSArray *)aValue iPadposition:(NSArray *)pos{
    
    [self.rightUtilityButtonsView setLableText:aValue iPadPosition:pos];
    NSLog(@"%@",self.rightUtilityButtonsView.text);
}
-(void)isPoratraitOrLandscape:(NSString *)orientation{
    [self.rightUtilityButtonsView setOrientationCheck:orientation];
}
